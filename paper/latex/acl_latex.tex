...
Paired skill-versus-no-skill runs across three capability tiers show a consistent pattern.
For the mid-tier glm-5.3-flash, the skill raises static-diagnosis scores from 77.8 to 87.0 points (mean task-level paired gain $+9.3$, 95\% CI $[+4.0, +16.1]$, two-sided Wilcoxon $p = 0.006$).
For the stronger glm-5.2, the unaided baseline is already near the ceiling and the gain is small and not significant ($+3.0$ points, $p = 0.138$).
For the locally served qwen3.8-27b, the skill shows a negative trend ($-3.1$ points, $p = 0.177$) while consuming 20.6\% more input tokens, 6.2\% more solver time, and 22 more timeouts, so the skill's context and instruction overhead outweighs its benefit on the weak model.
A model upgrade dominates the intervention: glm-5.2 without any skill already outperforms glm-5.3-flash with the skill, so at a fixed budget upgrading the model comes before adding a skill.
A four-condition source-matched design that would separate procedural organization from factual content is registered in the repository but had not been executed at submission time.
...
\end{quote}
The primary analysis is a task-level paired comparison of skill versus no-skill runs across three model capability tiers, with resource usage reported alongside correctness.
A follow-up question---whether organizing the same task-relevant facts as a skill adds value over supplying the source documents---is addressed by a registered four-condition design that had not been executed at submission time.
...
\item A task-level paired empirical analysis of skill utility across three model capability tiers, showing a significant gain for the mid-tier model, a ceiling-limited null for the stronger model, and a negative trend with higher resource consumption for the weak local model;
\item A registered four-condition source-matched control design, with an analysis framework separating functional correctness, diagnostic quality, and resource consumption, presented as a design contribution and future work rather than as executed results.
\end{itemize}
The benchmark, the paired runs, and the registered design are all available in the repository.
We distinguish executed results from registered-but-unexecuted designs throughout the draft.
...
\caption{Controlled study design (registered design, not yet executed at submission time). All conditions share the task and resource limits. C and D require a fact-coverage audit; sharing source files alone does not establish equivalence.}
\label{fig:design}
\end{figure}
...
Our focus is how the paired benefit of one migration skill varies across model capability tiers; a registered source-matched design, not yet executed, separately addresses how the skill's organization compares with the documents supplying its task-relevant facts.
...
A causal claim about prompt length or procedural organization would require an additional matched intervention, which the registered source-matched design provides for future execution.
...
For the registered controlled study, the primary endpoint is functional acceptance on hands-on tasks: whether the submitted artifact satisfies the task's required behavior at the resource deadline.
...
\section{A Registered Design for Source-Matched Control}
\label{sec:setup}
...
The design is registered in the repository: the study configuration, candidate material packages, runner, and statistical protocol all exist as versioned artifacts.
...
\subsection{Paired runs across three capability tiers}
\label{sec:paired}

Table~\ref{tab:paired-effect} reports the completed paired skill-versus-no-skill experiments.
The two GLM groups cover the 22 static-diagnosis tasks S1--S22 with three rounds per condition; each task contributes the median of its three rounds per arm.
The qwen3.8-27b group covers the full 56-task pool with three attempts per arm; each task contributes the mean of its scored rewards per arm, rescaled by 100 for comparability with the rubric-point scale (one unscored trial is excluded, never counted as zero).
...
The mid-tier glm-5.3-flash improves significantly with the skill: 77.8 to 87.0 points, mean paired gain $+9.27$, 95\% CI $[+3.95, +16.09]$, Wilcoxon $p = 0.0056$; 12 of 22 tasks tie and the interval excludes zero.
The stronger glm-5.2 starts at 93.3 points without the skill, leaving little headroom: the gain is $+3.05$ points with CI $[0, +7.18]$ and $p = 0.138$, consistent with a ceiling rather than evidence of absence.
The locally served qwen3.8-27b shows a negative trend, $-3.07$ points with CI $[-8.13, +1.99]$ and $p = 0.177$; because the interval crosses zero we report this as a trend, not an established regression.

A model upgrade dominates the intervention.
Descriptively, glm-5.2 without any skill (93.3) already exceeds glm-5.3-flash with the skill (87.0): at a fixed budget, moving to the stronger model bought more than injecting the skill into the weaker one.
This cross-model comparison is not paired and confounds model identity with capability, but the pattern is consistent within every tier we ran.
The task S19-phantom-update-stale-host illustrates the discrimination: glm-5.3-flash has a three-round median of 0 on both arms (one round scored 80/80), while glm-5.2 scored 100/100 on both arms in all three rounds, confirming the task separates capability rather than reflecting a rubric defect.

The weak model also pays more for the skill.
Its with-skill arm consumed 20.6\% more input tokens, 6.2\% more solver time, and timed out 22 more times than the no-skill arm (108 vs.\ 86 of 168 trials), so the skill's context and instruction overhead coincides with its negative trend.
For glm-5.3-flash, the round-1 report records with-skill sessions at roughly 2.2 times the session tokens of zero-skill sessions, because skill files enter every turn's context; here the overhead was affordable and the net effect remained positive.
...
\hline
\end{tabular}
\caption{Historical evidence categories. None supplies the matched source-document arm needed for the registered source-matched question.}
\label{tab:evidence}
\end{table}

Executing the registered four-condition design of Section~\ref{sec:setup} is future work; see Section~\ref{sec:discussion}.

\section{Analysis and Interpretation}
\label{sec:analysis}

\paragraph{Where the skill helps.}
The three tiers suggest that skill benefit is moderated by capability and budget rather than being a fixed property of the skill.
The mid-tier model has both the headroom and the capacity to convert guidance into correct diagnoses, and the gain is significant.
The stronger model's unaided baseline of 93.3 points leaves little room to improve, so the small, nonsignificant gain there should be read as ceiling-limited rather than as evidence that the skill stopped working.
The weak model's negative trend has several candidate mechanisms: the skill text crowds an already tight effective context budget, following the procedure imposes an instruction-following overhead the model cannot reliably pay, and a long skill document may simply distract a weak reader.
These are hypotheses, not established mechanisms; distinguishing them requires trajectory-level studies and matched interventions, not further score comparisons.

\paragraph{Resource tradeoffs.}
The realized consumption moved in the same direction as the effect.
The weak model's with-skill arm spent 20.6\% more input tokens and timed out 22 more times while scoring lower, and the mid-tier model paid roughly 2.2 times the session tokens for its significant gain.
...
The source-matched D--C contrast of the registered design remains the necessary next step for isolating the effect of procedural organization from that of factual content.
...
Executing the registered four-condition design of Section~\ref{sec:setup} is the natural next step: it separates the skill's procedural organization from its factual content, which the paired runs above cannot do.
A temporal holdout extends the same protocol to unseen incidents: ten clean tasks have been split out and the execution protocol is preregistered in the repository, including the schedule, bootstrap seed, and analysis policy.
Both are registered designs with committed artifacts; neither had been executed at submission time.
...
The preregistered temporal-holdout protocol described above would supply the separately frozen split and exposure audit a prospective incident study needs.
...
The paired results cover three models, one skill, and one ecosystem, and the source-matched four-condition experiment that would separate procedural organization from factual content remains unexecuted; all mechanism claims above are hypotheses bounded by that gap.
...
The GLM groups cover only the S-series static-diagnosis tasks; hands-on evidence comes solely from the qwen3.8-27b group, which used historical graders and a single agent protocol.
The original glm-5.2 round-3 artifacts are missing and its aggregate was reconstructed from the report's per-task table; the report's totals line contradicts that table by exactly 100 points per arm, and although the three-round medians used here are unaffected, the raw artifacts must be re-obtained from the run author.
The qwen3.8-27b negative effect has a confidence interval crossing zero and is reported as a trend only.
...
Exact model configurations, statistical grouping, and resource limits must be frozen before formal execution of the registered design.
...
Round aggregates and judge verdicts live under \texttt{benchmark/results/artifacts/2026-09-11-glm-5.3-flash-s1-s22*} and \texttt{benchmark/results/artifacts/2026-09-13-glm-5.2-s1-s22*}; the reconstructed glm-5.2 round-3 directory carries a \texttt{PROVENANCE.md} documenting the reconstruction and the unresolved totals discrepancy.
...
The frozen statistics file \texttt{benchmark/results/paired-effect-stats.json} and the deterministic script \texttt{benchmark/scripts/measure-paired-effect.mjs} (with golden tests and a byte-exact \texttt{--check} gate) reproduce every number in Table~\ref{tab:paired-effect}; \texttt{paper/generated/paired-effect-table.tex} is generated from the statistics file and must not be edited by hand.
