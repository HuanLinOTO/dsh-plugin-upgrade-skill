...
Paired skill-versus-no-skill runs at five model points spanning the capability range trace an inverted-U pattern.
At the weakest point, the locally served qwen3.8-27b shows a negative trend ($-3.1$ points, $p = 0.177$) while consuming 20.6\% more input tokens, 6.2\% more solver time, and 22 more timeouts: the skill's context and instruction overhead outweighs its benefit.
In the middle, three independent pairings gain $+8.7$ to $+10.7$ points (deepseek-v4-flash $+10.7$, gpt-5.6-terra $+8.7$, glm-5.3-flash $+9.3$), of which the three-round glm-5.3-flash protocol is the one that reaches significance (95\% CI $[+4.0, +16.1]$, Wilcoxon $p = 0.006$).
...
\end{quote}
The primary analysis is a task-level paired comparison of skill versus no-skill runs at five model points spanning the capability range, with resource usage reported alongside correctness.
...
\item A task-level paired empirical analysis of skill utility at five model points spanning the capability range---three multi-run protocols and two single-shot pairings---tracing an inverted-U: a negative trend at the weakest point, significant or near-significant gains in the middle, and a ceiling-limited null at the strongest point;
...
Table~\ref{tab:paired-effect} reports the completed paired skill-versus-no-skill experiments, ordered along the capability axis from the weakest model to the strongest.
...
The five points trace an inverted-U.
...
In the middle, three independent pairings all gain between $+8.7$ and $+10.7$ points.
...
glm-5.3-flash gains $+9.27$ and is the one middle-tier result that is significant on both measures: CI $[+3.95, +16.09]$, $p = 0.0056$.
At the strongest point, glm-5.2 starts at 93.3 points without the skill, leaving little headroom: the gain is $+3.05$ with CI $[0, +7.18]$ and $p = 0.138$, consistent with a ceiling rather than evidence of absence.
...
This cross-model comparison is not paired and confounds model identity with capability, but the pattern is consistent within every row we ran.
...
The weakest model also pays the most for the skill.
...
\paragraph{An inverted-U in capability.}
...
At the bottom, the weakest model cannot afford the skill: its with-skill arm spent 20.6\% more input tokens and timed out 22 more times while scoring lower.
Candidate mechanisms include the skill text crowding an already tight effective context budget, an instruction-following overhead the model cannot reliably pay, and simple distraction by a long document; these are hypotheses, not established mechanisms, and distinguishing them requires trajectory-level studies rather than further score comparisons.
The middle is the sweet spot: models with enough capacity to convert guidance into correct diagnoses but enough headroom to show it all gained $+8.7$ to $+10.7$ points, across three different harnesses, task pools, and grader generations.
At the top, the strongest model's unaided baseline of 93.3 points leaves little room to improve, so its small, nonsignificant gain should be read as ceiling-limited rather than as evidence that the skill stopped working.

\paragraph{Directional corroboration.}
Three repository-held results outside the main table point the same way, each with its own caveat.
...
None of these enters the main table; they are consistent context, not evidence.

\paragraph{Resource tradeoffs.}
The realized consumption moved in the same direction as the effect.
The weakest model's with-skill arm paid more tokens and timeouts while scoring lower, and the mid-tier model paid roughly 2.2 times the session tokens for its significant gain.
...
The groups are not apples-to-apples: task pools differ (22, 23, and 56 tasks), protocols differ (three-round and three-run medians versus single-shot), and harnesses and grader generations differ, so the cross-row inverted-U is a descriptive pattern and only the within-row comparisons are exact.
...
The gpt-5.6-luna pairing is excluded from the main table because its no-skill arm is not a literal zero-skill run: Harbor mounted no skill, but the agent's native system-skill catalog stayed visible, and the H2 and H3 trajectories explicitly read the native plugin-creator skill.
Both contaminated scores are perfect, so the nominal $+15.17$-point gain in Table~\ref{tab:paired-effect-sensitivity} is, if anything, understated relative to a clean baseline; the decontaminated 16-task no-skill subset (mean 0.6513) is diagnostic only and is not a substitute for rerunning those tasks with native skills disabled.
