...
| 14 | [v0.1.6-alpha.1.md](v0.1.6-alpha.1.md) | `dsh-v0.1.5-rc.2` | `dsh-v0.1.6-alpha.1` | 38 | draft / curated (the widest edge carded here so far: 800 commits, 4015 changed files, 307 of them `package.json` version strings. Host: `agent/session-start` deleted and `agent/created` becomes a serial awaited event carrying `source`/`signal`, startup strictness moves to `auditStartupEntries` so a resolved `Loader.create()` no longer proves activation, `Session.eventAt()`/`snapshotEvents()`/`ownEvents()` are deprecated and lint-protected, `ctx.sessions.registerMessageProjection()` arrives with a refusal for content-rewriting events that lack a registered pure interpreter, and `session-log-deepseek` uploads by default. Runtime: `ctx.codeRuntime` → `ctx.ptcRuntime` with `CodeRuntime` → `PtcRuntime` and `run(request)` split into `resolve(request)` + `run(spec)`; `SandboxProvider.confine` and `ShellExecutor.start` become asynchronous; subprocess providers must implement `terminalEnvironment()` and may serve a control channel; `workflow-ptc` replaces the worker-thread workflow provider and requires a PTC runtime plus sandbox policy; the MCP client moves to the 2.0 SDK; new `ctx.mcpResources` and the `ctx.ssh` execution world. LLM: the adapter image budget now reports `IMAGE_OFFLOAD_REQUIRED` instead of projecting images, `deepseek-official` defaults to the Anthropic Messages protocol, request images move to the v41 token grid, token-meter and both context projections bump `stateVersion` to 5, `AssistantProvenance` → `AssistantProviderMetadata`. Web Client: context "provenance" becomes "producer" and assistant "provenance" becomes "provider metadata", `CommandClaim` requires `name`, the composer permission control moves to the new `conversation.input.permission` slot, right-sidebar guide entries require `id` with a keyed `sidebar.right.tab.guide.entry` slot, `ConnectionIndicator` loses `reconnectLabel`, diffs become context-aware, and the standalone `?fixture` page mode is replaced by the client assembly tier. Composition and packaging: the base swaps `workflow-worker-thread` for a `ptc-runtime` + `workflow-ptc` pair, mounts `image-offload` (whose `image/offload` event is required on read) and `mcp-resources`, disables `tool-ralph` by default, the Web app bundle drops the `code-runtime` row and mounts three new rows, the workspace ledger adds 22 packages and retires 7, headless gains `--session-id`/`--json`, the public package manifest type is rebuilt, experimental packages publish by default behind an empty denylist, and the `node-addon-require-builtin` floor moves `^0.1.4` → `^0.1.6`; negative evidence records that `SESSION_FORMAT_VERSION` (3), the engine floor and the launcher CLI grammar did not move) |
| 16 | [v0.1.7-alpha.1.md](v0.1.7-alpha.1.md) | `dsh-v0.1.6-alpha.2` | `dsh-v0.1.7-alpha.1` | 10 | draft / curated (client primitives icon set renamed - every `*16` export removed in favor of `*Medium`/`*Regular` variants, with first-hand fleet evidence of a whole-slot silent vanish via React #130; `workspaceFiles` reads unify on `readBytes`; settings move into the current Profile's plugin configuration with a one-time `settings.yaml` import; agent presets become bundle-declared; session log V4 with a batch migration tool; the official DeepSeek adapter goes Messages-API-only; ordered multi-patch bundles and no-reload config fields; tolerant Profile loading; locale-aware plugin metadata; a custom-event attachment adaptation duty. Corridor dependency: the preceding edge `dsh-v0.1.6-alpha.1 → dsh-v0.1.6-alpha.2` is carded separately as order 15, `v0.1.6-alpha.2.md`) |
| 17 | [v0.1.7-alpha.2.md](v0.1.7-alpha.2.md) | `dsh-v0.1.7-alpha.1` | `dsh-v0.1.7-alpha.2` | 4 | draft / curated (`spill-policy` moves `maxInlineBytes` to `maxInlineTokens` with a shared token budget retaining head and tail; vendored dependency auto-updates pin to same-minor patches; first plugin install auto-selects the most responsive npm registry; Web pages auto-reconnect after a server restart and client bundles are cached at host boot — an install-tree client edit now needs a host restart before the page sees it, first-hand evidence. Stacks on [v0.1.7-alpha.1.md](v0.1.7-alpha.1.md)) |
| 18 | [v0.1.7-rc.1.md](v0.1.7-rc.1.md) | `dsh-v0.1.7-alpha.2` | `dsh-v0.1.7-rc.1` | 5 | draft / curated (DSH peer ranges become enforced at install and at startup with exact-version exemptions in `<profile>/compatibility.json` — under the `semver ^7.8.5` the host uses (the caret change landed in semver 7.8.3), `^0.1.7`/`~0.1.7`/`>=0.1.7` do NOT admit `0.1.7-rc.1`, and a denied row still shows in `--dump-config`, first-hand evidence; `tool.call.toolview` owners become a `preparing`/`start`/`result` phase union with `argsRaw` only on started calls plus a streaming-arguments hook; Conversation Definitions accept transient starts and treat a repeated start as an update; the work-detail setting gains four modes with `standard` as the default; Plugin Manager bounds silent pnpm runs and prechecks GitHub reachability. Stacks on [v0.1.7-alpha.2.md](v0.1.7-alpha.2.md)) |
| — | [jump-0.1.5-rc.2-to-0.1.7-rc.1.md](jump-0.1.5-rc.2-to-0.1.7-rc.1.md) | `dsh-v0.1.5-rc.2` | `dsh-v0.1.7-rc.1` | 33 | jump card set (`kind: dsh-version-jump-card-set`, `DSH-0.1.7-J1`; schema-validated like a card file, but not a corridor edge): the net plugin-relevant change of the `0.1.5-rc.2 → 0.1.7-rc.1` corridor (~3300 commits) for a deployment that never stopped on the alphas. Covers the host/runtime plane — DSH peer-compatibility enforcement at install and startup, the PTC runtime rename, declarative agent presets, settings moving into the profile plugin Config, Session log V4 + attachment-carrier narrowing, Remote duplex/binary + `readBytes`, the async Agent lifecycle, the `Message` role union, the Messages-only DeepSeek adapter, subagent config/depth, E2B removal + SSH providers, Ralph off, token-budgeted spill-policy, read-only `tool-cordis`, bundle patch lists, the package-rename ledger, CLI/headless surfaces, Agent Team single bundle, experimental speech/browser/computer-use, bundled LibreOffice, MCP resources, session-log privacy default — and the Web Client plane: the toolview three-phase union, chat-node/turnTail reshape, presentation-policy channel, icon weight renames, `settingsScope`→`configForms`, the right-sidebar tab contract, the new sidebar packages, component factories, the plugin-manager page + `pluginManager` Remote, `ModelDirectory.select` results, tool-cordis `@pluginId` removal, the projection-driven Team panel, markdown file images, sessions/`jobs` references and the new client transport. The per-edge cards for the same window are orders 14–18; this is the jump view, not a per-edge ledger, and duplicated cards cross-reference their per-edge IDs) |
...
A version-jump companion (`jump-<from>-to-<to>.md`) uses the same frontmatter and card format
with `kind: dsh-version-jump-card-set` and its own ID prefix (for example `DSH-0.1.7-J1`). It is
validated like a card file, and its IDs share the repository-wide namespace, but it is not a
corridor edge: `scripts/plan-migration.mjs` only walks `v*.md` files.
