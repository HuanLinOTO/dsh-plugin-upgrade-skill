...
| 14 | [v0.1.7-alpha.1.md](v0.1.7-alpha.1.md) | `dsh-v0.1.6-alpha.2` | `dsh-v0.1.7-alpha.1` | 10 | draft / curated (client primitives icon set renamed - every `*16` export removed in favor of `*Medium`/`*Regular` variants, with first-hand fleet evidence of a whole-slot silent vanish via React #130; `workspaceFiles` reads unify on `readBytes`; settings move into the current Profile's plugin configuration with a one-time `settings.yaml` import; agent presets become bundle-declared; session log V4 with a batch migration tool; the official DeepSeek adapter goes Messages-API-only; ordered multi-patch bundles and no-reload config fields; tolerant Profile loading; locale-aware plugin metadata; a custom-event attachment adaptation duty. Corridor dependency: the preceding edge `0.1.6-alpha.1 -> 0.1.6-alpha.2` is covered by PR #248) |
| 15 | [v0.1.7-alpha.2.md](v0.1.7-alpha.2.md) | `dsh-v0.1.7-alpha.1` | `dsh-v0.1.7-alpha.2` | 4 | draft / curated (`spill-policy` moves `maxInlineBytes` to `maxInlineTokens` with a shared token budget retaining head and tail; vendored dependency auto-updates pin to same-minor patches; first plugin install auto-selects the most responsive npm registry; Web pages auto-reconnect after a server restart and client bundles are cached at host boot — an install-tree client edit now needs a host restart before the page sees it, first-hand evidence. Stacks on [v0.1.7-alpha.1.md](v0.1.7-alpha.1.md) / PR #252) |
| 16 | [v0.1.7-rc.1.md](v0.1.7-rc.1.md) | `dsh-v0.1.7-alpha.2` | `dsh-v0.1.7-rc.1` | 5 | draft / curated (DSH peer ranges become enforced at install and at startup with exact-version exemptions in `<profile>/compatibility.json` — under the `semver ^7.8.5` the host uses, `^0.1.7`/`~0.1.7`/`>=0.1.7` do NOT admit `0.1.7-rc.1`, and a denied row still shows in `--dump-config`, first-hand evidence; `tool.call.toolview` owners become a `preparing`/`start`/`result` phase union with `argsRaw` only on started calls plus a streaming-arguments hook; Conversation Definitions accept transient starts and treat a repeated start as an update; the work-detail setting gains four modes with `standard` as the default; Plugin Manager bounds silent pnpm runs and prechecks GitHub reachability. Stacks on [v0.1.7-alpha.2.md](v0.1.7-alpha.2.md) / PR #252) |
