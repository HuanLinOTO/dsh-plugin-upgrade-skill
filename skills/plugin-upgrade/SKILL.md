...
| [references/v0.1.6-alpha.1.md](references/v0.1.6-alpha.1.md) | 0.1.5-rc.2→0.1.6-alpha.1 draft cards (38, the widest edge carded here: 800 commits / 4015 files). Host: `agent/session-start` deleted and `agent/created` becomes serial + awaited, `auditStartupEntries` replaces the `assertEntries*` pair, session event reads deprecated, `registerMessageProjection()` with a refusal when no pure interpreter is registered, `session-log-deepseek` uploads by default. Runtime: `codeRuntime`→`ptcRuntime` with a `resolve`/`run` split, async `SandboxProvider.confine` and `ShellExecutor.start`, subprocess `terminalEnvironment()` + optional control channel, `workflow-ptc` replaces the worker-thread provider, MCP 2.0 SDK, new `ctx.mcpResources` and `ctx.ssh` capabilities. LLM: adapters report `IMAGE_OFFLOAD_REQUIRED` instead of projecting images, `deepseek-official` defaults to the Anthropic Messages protocol, images move to the v41 token grid, projection `stateVersion` 5, `AssistantProvenance`→`AssistantProviderMetadata`. Web Client: producer/provider renames, required `CommandClaim.name`, new `conversation.input.permission` and keyed guide-entry slots, `ConnectionIndicator.reconnectLabel` removed, context-aware diffs, `?fixture` mode replaced by the assembly tier. Composition/packaging: base swaps `workflow-worker-thread` for `ptc-runtime` + `workflow-ptc`, mounts `image-offload` (required-on-read `image/offload`) and `mcp-resources`, disables `tool-ralph` by default, the Web bundle drops the `code-runtime` row, a +22/−7 package ledger, headless `--session-id`/`--json`, public package manifest rebuild, experimental publication flipped to a denylist, `node-addon-require-builtin` floor `^0.1.4`→`^0.1.6`; negative evidence that `SESSION_FORMAT_VERSION` (3), the engine floor and the launcher CLI grammar did not move |
| [references/v0.1.7-alpha.1.md](references/v0.1.7-alpha.1.md) | 0.1.6-alpha.2→0.1.7-alpha.1 draft cards (10): the client primitives icon set drops every `*16` export for `*Medium`/`*Regular` variants (a missing export vanishes the whole slot entry via React #130), `workspaceFiles` reads unify on `readBytes`, settings move into the current Profile's plugin configuration with a one-time `settings.yaml` import, bundle-declared agent presets, session log V4 with a batch migration tool, the official DeepSeek adapter is Messages-API only, ordered multi-patch bundles and no-reload config fields, tolerant Profile loading, locale-aware plugin metadata, and custom-event attachments no longer auto-read. The preceding edge `0.1.6-alpha.1→0.1.6-alpha.2` is carded separately |
| [references/v0.1.7-alpha.2.md](references/v0.1.7-alpha.2.md) | 0.1.7-alpha.1→0.1.7-alpha.2 draft cards (4): `spill-policy` `maxInlineBytes` becomes `maxInlineTokens` with a shared head/tail token budget, vendored dependency auto-updates pin to same-minor patches, the first plugin install auto-selects the most responsive npm registry, and client bundles are cached at host boot (an install-tree client edit needs a host restart) while Web pages reconnect after a server restart |
| [references/v0.1.7-rc.1.md](references/v0.1.7-rc.1.md) | 0.1.7-alpha.2→0.1.7-rc.1 draft cards (5): DSH peer ranges are enforced at install and startup with exact-version exemptions in `<profile>/compatibility.json` — with the host's semver `^7.8.5` (caret change since 7.8.3) and `includePrerelease`, `^0.1.7`/`~0.1.7`/`>=0.1.7` refuse `0.1.7-rc.1`, and `--dump-config` still lists a denied row; `tool.call.toolview` owners become a `preparing`/`start`/`result` phase union with `argsRaw` only on started calls; Conversation Definitions accept transient starts; the work-detail setting gains four modes (default `standard`); Plugin Manager bounds silent pnpm runs and prechecks GitHub reachability |
| [references/jump-0.1.5-rc.2-to-0.1.7-rc.1.md](references/jump-0.1.5-rc.2-to-0.1.7-rc.1.md) | 0.1.5-rc.2→0.1.7-rc.1 version-jump curated cards (33, `DSH-0.1.7-J1`): host/runtime plane — DSH peer-compatibility enforcement at install and startup with exact-version exemptions, PTC runtime rename (`codeRuntime`→`ptcRuntime`, Node engine in a separate process), declarative agent presets (`dsh-agent-presets` split), settings into the profile plugin Config (`settings.register` removed), Session log V4 + `session-format-v3-to-v4` + attachment-carrier narrowing, Remote duplex/binary + `readBytes`, async Agent lifecycle (`agent/session-start` removed, `agent/created` gains `source`), the `Message` role union + `createSystemMessage(text)`, Messages-only DeepSeek adapter (`/anthropic`), subagent Config/depth, E2B removal + POSIX SSH providers, Ralph off, token-budgeted spill-policy, read-only `tool-cordis`, bundle ordered patch lists, the package-rename ledger, new CLI/headless surfaces, Agent Team single bundle, experimental speech/browser/computer-use, bundled LibreOffice + packaged-app Node requirement, MCP resources, session-log privacy default; Web Client plane — toolview `preparing` phase, chat-node/turnTail reshape, presentation-policy channel, `*16`→`*Regular`/`*Medium` icon renames, `settingsScope`→`configForms`, right-sidebar tab contract, new sidebar packages, component factories, plugin-manager page + `pluginManager` Remote, `ModelDirectory.select` results, `ui-cordis` `@pluginId` removal, session-row action slots. The per-edge 0.1.6/0.1.7 card files cover the same window edge by edge; this file is the jump view, not a corridor edge, and is not read by `plan-migration.mjs` |
