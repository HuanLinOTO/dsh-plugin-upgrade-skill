...
| [references/v0.1.7-alpha.1.md](references/v0.1.7-alpha.1.md) | 0.1.6-alpha.2→0.1.7-alpha.1 draft 卡（10 张）：客户端 primitives 图标集删除全部 `*16` 导出、改用 `*Medium`/`*Regular` 变体（缺失导出会经 React #130 让整个 slot 条目静默消失）、`workspaceFiles` 读取统一到 `readBytes`、设置迁入当前 Profile 的插件配置并一次性导入 `settings.yaml`、agent preset 改由 bundle 声明、session log V4 及批量迁移工具、官方 DeepSeek 适配器只走 Messages API、有序多 patch bundle 与免重载配置字段、容错 Profile 加载、按 locale 的插件元数据、仅存在于自定义事件的附件不再自动读取。前一条边 `0.1.6-alpha.1→0.1.6-alpha.2` 单独成卡 |
| [references/v0.1.7-alpha.2.md](references/v0.1.7-alpha.2.md) | 0.1.7-alpha.1→0.1.7-alpha.2 draft 卡（4 张）：`spill-policy` 的 `maxInlineBytes` 改为 `maxInlineTokens`（首尾共享 token 预算）、vendored 依赖自动更新只锁同 minor 补丁、首次安装插件自动选最快的 npm registry、客户端 bundle 在宿主启动时缓存（改安装树里的客户端文件后需重启宿主）且服务重启后 Web 页面自动重连 |
| [references/v0.1.7-rc.1.md](references/v0.1.7-rc.1.md) | 0.1.7-alpha.2→0.1.7-rc.1 draft 卡（5 张）：DSH peer 范围在安装和启动时强制校验，可在 `<profile>/compatibility.json` 里按精确版本豁免——宿主用 semver `^7.8.5`（caret 行为自 7.8.3 起改变）并带 `includePrerelease`，`^0.1.7`/`~0.1.7`/`>=0.1.7` 都不接受 `0.1.7-rc.1`，且被拒的行在 `--dump-config` 里照样显示；`tool.call.toolview` owner 变成 `preparing`/`start`/`result` 阶段联合、`argsRaw` 只在已开始的调用上；Conversation Definition 接受瞬态事件启动；工作详情设置改为四种模式（默认 `standard`）；插件管理器限制静默 pnpm 运行并预检 GitHub 连通性 |
| [references/jump-0.1.5-rc.2-to-0.1.7-rc.1.md](references/jump-0.1.5-rc.2-to-0.1.7-rc.1.md) | 0.1.5-rc.2→0.1.7-rc.1 版本跳跃 curated 卡（33 张，`DSH-0.1.7-J1`）：宿主/运行时面——DSH peer 兼容性在安装和启动时强制校验并支持精确版本豁免、PTC 运行时改名（`codeRuntime`→`ptcRuntime`，Node 引擎移到独立进程）、agent preset 改为声明式（`dsh-agent-presets` 拆分）、设置迁入 profile 插件 Config（`settings.register` 移除）、Session log V4 + `session-format-v3-to-v4` + 附件载体收窄、Remote 双工/二进制传输 + `readBytes`、Agent 生命周期异步化（`agent/session-start` 移除、`agent/created` 带 `source`）、`Message` 角色联合与 `createSystemMessage(text)`、DeepSeek 官方适配器只走 Messages（`/anthropic`）、subagent Config/深度、E2B 移除改用 POSIX SSH provider、Ralph 默认关闭、spill-policy 改按 token 预算、`tool-cordis` 只读、bundle 有序 patch 列表、包改名台账、新 CLI/headless 面、Agent Team 单 bundle、实验性语音/浏览器/computer-use、内置 LibreOffice 与打包应用的 Node 要求、MCP resources、session-log 隐私默认值；Web Client 面——toolview `preparing` 阶段、chat-node/turnTail 重塑、presentation-policy 通道、`*16`→`*Regular`/`*Medium` 图标改名、`settingsScope`→`configForms`、右侧栏标签契约、新侧栏包、组件工厂、插件管理页 + `pluginManager` Remote、`ModelDirectory.select` 返回值、`ui-cordis` 去掉 `@pluginId`、会话行动作 slot。同一窗口的逐边卡片见 0.1.6/0.1.7 各版本卡文件；本文件是跳跃视图，不是走廊边，`plan-migration.mjs` 不读取 |
